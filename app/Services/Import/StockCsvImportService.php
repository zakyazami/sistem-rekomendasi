...
final class StockCsvImportService
{
    ...
    public function commit(string $path, ?User $user = null): StockImportSummary
    {
        ...
        return DB::transaction(function () use ($analysis, $path, $user, $preview): StockImportSummary {
            ...
            $existing = StockHistory::query()
                ->whereIn('product_id', $productIds)
                ->get()
                ->keyBy(static fn (StockHistory $history): string => $history->product_id.'|'.$history->date->toDateString());
            $upserts = [];
            $inserted = 0;
            $updated = 0;
            $skipped = 0;
            ...
            foreach (array_chunk($upserts, 1000) as $chunk) {
    ...
    private function analyze(string $path): array
    {
        ...
        while (! $file->eof()) {
            ...
            $key = $row['product_normalized'].'|'.$row['date'];
            if (isset($seen[$key])) {
                throw ValidationException::withMessages([
            ...
            $products[$row['product_normalized']] = [
                'name' => $row['product'],
        ...
        $this->validateContinuity($rows, $errors);
        ...
        return compact('summary', 'rows', 'categories', 'products');
    }
    ...
    /** @param list<array<string, int|string>> $rows @param list<string> $errors */
    private function validateContinuity(array $rows, array &$errors): void
    {
        $grouped = collect($rows)->groupBy('product_normalized');

        foreach ($grouped as $productRows) {
            $ordered = $productRows->sortBy('date')->values();

            for ($index = 1; $index < $ordered->count(); $index++) {
                if ($ordered[$index]['initial_stock'] !== $ordered[$index - 1]['final_stock']) {
                    $errors[] = sprintf(
                        'Kontinuitas stok %s pada %s tidak sesuai.',
                        $ordered[$index]['product'],
                        $ordered[$index]['date'],
                    );
                }
            }
    ...
    private function sameStockValues(StockHistory $history, array $values): bool
    {
